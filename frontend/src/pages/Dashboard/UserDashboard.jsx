...
export default function UserDashboard() {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            {!user?.is_paid && user?.role === 'user' && (
                <div className="card rounded-3xl p-6 bg-brand-600/5 border-brand-500/20 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-brand-500/20 rounded-2xl flex items-center justify-center text-brand-400">
                            <Zap size={24} />
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-surface-100 italic">Free Tier Active</h3>
                            <p className="text-sm text-surface-400 font-medium">You have used {user?.validation_count || 0} of 10 available validations</p>
                        </div>
                    </div>
                    <div className="w-full md:w-64">
                        <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2">
                            <span className="text-brand-400">Usage Status</span>
                            <span className="text-surface-500">{Math.min(((user?.validation_count || 0) / 10) * 100, 100).toFixed(0)}%</span>
                        </div>
                        <div className="h-2 bg-surface-800 rounded-full overflow-hidden">
                            <div
                                className={`h-full transition-all duration-1000 ${(user?.validation_count || 0) >= 10 ? 'bg-danger-500' : 'bg-brand-500'}`}
                                style={{ width: `${Math.min(((user?.validation_count || 0) / 10) * 100, 100)}%` }}
                            />
                        </div>
                    </div>
                </div>
                                    ...
                                    <div className="flex items-center gap-4 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-surface-500 font-medium mt-0.5 flex items-center gap-1.5">
                                                ...
                                                <span>Score: {(result.final_score * 100).toFixed(1)}%</span>
                                            </p>
                                        </div>
                                    </div>
